...
<div class="row g-3">
    ...
    <div class="col-12 col-xl-9">
        ...
        {{-- Top Role Header Card --}}
        <div class="card mb-3 border-0 shadow-sm">
            <div class="card-body p-4">
                <div class="d-flex align-items-center justify-content-between flex-wrap gap-3">
                    <div class="d-flex align-items-center gap-3">
                        <div class="avatar avatar-3xl rounded-circle text-white d-flex align-items-center justify-content-center fw-bold fs-2" style="width: 60px; height: 60px; background-color: #6f42c1;">
                            {{ strtoupper(substr($role['name'], 0, 1)) }}
                        </div>
                        <div>
                            <div class="d-flex align-items-center gap-2">
                                <h4 class="mb-0 fw-bold text-dark">{{ $role['name'] }}</h4>
                                <span class="badge text-white px-3 py-1 rounded-pill fw-semibold" style="background-color: #6f42c1;">Role Permissions</span>
                            </div>
                            <small class="text-muted">
                                <i class="fas fa-user-tag me-1"></i>Role ID: {{ $role['id'] }} &nbsp;|&nbsp;
                                <i class="fas fa-info-circle me-1"></i>Permissions granted here apply to all users assigned this role.
                            </small>
                        </div>
                    </div>
                    <div>
                        <a href="{{ route('roles') }}" class="btn btn-outline-secondary btn-sm px-3">
            ...
            <div class="card-body p-4">
                <div class="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
                    <div>
                        ...
                        <small class="text-muted">Choose the module sections for this role</small>
                    </div>
    ...
    <div class="col-12 col-xl-3">
        <div class="card border-0 shadow-sm sticky-summary">
            <div class="card-header text-white py-3" style="background-color: #6f42c1;">
                <h6 class="mb-0 text-white fw-bold"><i class="fas fa-shield-alt me-2"></i>Role Permissions Summary</h6>
            </div>
            <div class="card-body p-3">
                ...
                <div class="mb-3 p-3 bg-light rounded text-center border">
                    ...
                    <div class="progress" style="height: 8px;">
                        <div class="progress-bar" id="barAccessLevel" role="progressbar" style="width: 0%; background-color: #6f42c1;"></div>
                    </div>
                </div>
                ...
                <div class="alert alert-info p-2 small mb-3 border-0 bg-info-soft text-info">
                    <i class="fas fa-info-circle me-1"></i> Role permissions update automatically for all admins assigned to this role.
                </div>

                <div class="d-grid gap-2">
                    <button type="button" class="btn text-white" id="btnSavePermissions" style="background-color: #6f42c1;">
                        <i class="fas fa-save me-1"></i> Save Role Permissions
                    </button>
                    <a href="{{ route('roles') }}" class="btn btn-outline-secondary btn-sm">
                        <i class="fas fa-arrow-left me-1"></i> Return to Roles List
                    </a>
                </div>
...
$(function () {
    ...
    let roleId = "{{ $role['id'] }}";
    let updateUrl = "{{ route('roles.permission.update') }}";
    ...
    $(document).on('change', '.cb-op, .cb-allow-all', function () {
        ...
        $.ajax({
            ...
            error: function () {
                ...
                toastr.error("Failed to update role permission");
            }
        });
    });
    ...
    $(document).on('change', '.cb-custom-action', function () {
        ...
        $.ajax({
            ...
            success: function (res) {
                updateMetrics();
                toastr.success("Role sub-permission updated successfully");
            },
            error: function () {
                cb.prop('checked', !isChecked);
                toastr.error("Failed to update role sub-permission");
            }
        });
    });
    ...
    $(document).on('click', '.btn-cat-all', function () {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                updateMetrics();
                toastr.success(val === 1 ? "Category permissions enabled for role" : "Category permissions disabled for role");
            }
        });
    });
    ...
    $('#btnSelectAllSections, #btnUnselectAllSections').click(function () {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                updateMetrics();
                toastr.success(val === 1 ? "All permissions granted to role" : "All permissions revoked from role");
            }
        });
    });
    ...
    $('#btnSavePermissions').click(function () {
        toastr.success("Role permissions saved & applied to assigned admins!");
    });
