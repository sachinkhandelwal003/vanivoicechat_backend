...
<div class="container">
    <div class="card">
        ...
        <div class="card-body">
            ...
            <form action="{{ route('frame.edit', $frame->id) }}" method="POST" enctype="multipart/form-data">
                @csrf

                <div class="mb-3">
                    <label class="form-label fw-bold">Frame Name <span class="text-danger">*</span></label>
                ...
                <div class="mb-3">
                    ...
                    <select name="visibility_type" id="visibilityType" class="form-control">
                        <option value="backend" {{ $frame->visibility_type === 'backend' ? 'selected' : '' }}>
                            Backend
                        </option>
                        <option value="in_app" {{ $frame->visibility_type === 'in_app' ? 'selected' : '' }}>
                            In App
                        </option>
                    </select>
                ...
                <div id="inAppFields" class="{{ $frame->visibility_type === 'in_app' ? '' : 'd-none' }}">

                    @php
                    $needcoins = old('needcoin', $frame->needcoin ?? []);
                    $validities = old('validity', $frame->validity ?? []);
                    ...
                    <!-- <div id="inAppFields"
                        class="{{ old('visibility_type', $frame->visibility_type) === 'in_app' ? '' : 'd-none' }}"> -->
                ...
                <div class="mb-4">
                    ...
                    <label for="icon"
                        ...
                        <img id="coverPreview"
                            src="{{ $frame->icon ? Helper::showImage($frame->icon, true) : '' }}"
                            class="position-absolute w-100 h-100 {{ $frame->icon ? '' : 'd-none' }}"
                            style="object-fit:cover;">

                        <span id="coverPlus" class="fs-1 {{ $frame->icon ? 'd-none' : '' }}">+</span>
                    </label>
                ...
                <div class="mb-4">
                    ...
                    <small class="d-block mt-1 text-muted">
                        Current File:
                        <a href="{{ Helper::showImage($frame->gif, true) }}" target="_blank">
                ...
                <div class="mb-3">
                    ...
                    <select name="status" class="form-control">
                        <option value="1" {{ $frame->status == '1' ? 'selected' : '' }}>Enable</option>
                        <option value="0" {{ $frame->status == '0' ? 'selected' : '' }}>Disable</option>
                    </select>
                </div>

                <div class="d-flex justify-content-end gap-2">
                    <a href="{{ route('frame') }}" class="btn btn-secondary">Cancel</a>
