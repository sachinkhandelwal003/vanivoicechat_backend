            ...
            <form action="{{ route('cars.edit', $car->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <select name="visibility_type" id="visibilityType" class="form-control">
                        <option value="backend" {{ $car->visibility_type === 'backend' ? 'selected' : '' }}>
                            Backend
                        </option>
                        <option value="in_app" {{ $car->visibility_type === 'in_app' ? 'selected' : '' }}>
                            In App
                        </option>
                    </select>
                ...
                <div id="inAppFields" class="{{ $car->visibility_type === 'in_app' ? '' : 'd-none' }}">

                    @php
                    $needcoins = old('needcoin', $car->needcoin ?? []);
                    $validities = old('validity', $car->validity ?? []);
                    ...
                    <!-- <div id="inAppFields"
                        class="{{ old('visibility_type', $car->visibility_type) === 'in_app' ? '' : 'd-none' }}"> -->
                ...
                <div class="mb-4">
                    ...
                    <label for="icon"
                        ...
                        <img id="coverPreview"
                            src="{{ $car->icon ? Helper::showImage($car->icon, true) : '' }}"
                            class="position-absolute w-100 h-100 {{ $car->icon ? '' : 'd-none' }}"
                            style="object-fit:cover;">

                        <span id="coverPlus" class="fs-1 {{ $car->icon ? 'd-none' : '' }}">+</span>
                    </label>
                ...
                <div class="mb-4">
                    <label class="form-label fw-bold">
                        Animation (Gif) <span class="text-danger">*</span>
                    </label>
                    ...
                    <small class="d-block mt-1 text-muted">
                        Current File:
                        <a href="{{ Helper::showImage($car->gif, true) }}" target="_blank">
                            View Animation
                        </a>
                    </small>
                    @endif
                ...
                <div class="mb-3">
                    ...
                    <select name="status" class="form-control">
                        <option value="1" {{ $car->status == '1' ? 'selected' : '' }}>Enable</option>
                        <option value="0" {{ $car->status == '0' ? 'selected' : '' }}>Disable</option>
                    </select>
                </div>
