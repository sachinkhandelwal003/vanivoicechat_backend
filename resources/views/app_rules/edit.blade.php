@extends('layouts.app')

@section('content')
<div class="container-fluid">

    <div class="card">
        <div class="card-header">
            <h5 class="mb-0">Edit App Rule</h5>
        </div>

        <div class="card-body">
            <form action="{{ route('app-rules.update', $rule->id) }}" method="POST" id="editRuleForm">
                @csrf

                <div class="mb-3">
                    <label class="form-label">Heading <span class="text-danger">*</span></label>
                    <input type="text"
                        name="heading"
                        class="form-control"
                        placeholder="Enter rule heading"
                        value="{{ old('heading', $rule->heading) }}"
                        required>
                </div>


                <div class="mb-3">
                    <label class="form-label">Type <span class="text-danger">*</span></label>
                    <input type="text" name="type" class="form-control"
                        placeholder="Enter rule Type" value="{{ old('type', $rule->type) }}" required>
                    <span style="color: red;">Note:Do not give space</span>
                </div>

                <div class="mb-3">
                    <label class="form-label">Rule <span class="text-danger">*</span></label>
                    <textarea name="rule"
                        id="rule"
                        class="form-control"
                        rows="8"
                        placeholder="Enter rule content">{{ old('rule', $rule->rule) }}</textarea>
                </div>

                <div class="mb-3">
                    <label class="form-label">Status</label>
                    <select name="status" class="form-control">
                        <option value="1" {{ old('status', $rule->status) == 1 ? 'selected' : '' }}>Active</option>
                        <option value="0" {{ old('status', $rule->status) == 0 ? 'selected' : '' }}>Inactive</option>
                    </select>
                </div>

                <div class="d-flex justify-content-end gap-2">
                    <a href="{{ route('app-rules.index') }}" class="btn btn-secondary">
                        Cancel
                    </a>

                    <button type="submit" class="btn btn-primary">
                        Update Rule
                    </button>
                </div>

            </form>
        </div>
    </div>

</div>
@endsection

@push('scripts')
<link href="https://cdn.jsdelivr.net/npm/summernote@0.8.20/dist/summernote-lite.min.css" rel="stylesheet">
<script src="https://cdn.jsdelivr.net/npm/summernote@0.8.20/dist/summernote-lite.min.js"></script>

<script>
    $(document).ready(function() {
        $('#rule').summernote({
            height: 300,
            placeholder: 'Enter rule content...',
            toolbar: [
                ['style', ['style']],
                ['font', ['bold', 'italic', 'underline', 'clear']],
                ['fontname', ['fontname']],
                ['fontsize', ['fontsize']],
                ['color', ['color']],
                ['para', ['ul', 'ol', 'paragraph']],
                ['insert', ['link', 'picture']],
                ['view', ['fullscreen', 'codeview']]
            ]
        });
    });
</script>
@endpush