...
<div class="container">
    <div class="card">
        ...
        <div class="card-body">
            ...
            <form action="{{ route('theme.edit', $theme->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="mb-3">
                    ...
                    <select name="visibility_type" id="visibilityType" class="form-control">
                        <option value="backend" {{ $theme->visibility_type === 'backend' ? 'selected' : '' }}>
                            Backend
                        </option>
                        <option value="in_app" {{ $theme->visibility_type === 'in_app' ? 'selected' : '' }}>
                            In App
                        </option>
                    </select>
                ...
                <div id="inAppFields" class="{{ $theme->visibility_type === 'in_app' ? '' : 'd-none' }}">

                    @php
                    $needcoins = old('needcoin', $theme->needcoin ?? []);
                    $validities = old('validity', $theme->validity ?? []);
                    ...
                    <!-- <div id="inAppFields"
                        class="{{ old('visibility_type', $theme->visibility_type) === 'in_app' ? '' : 'd-none' }}"> -->
                ...
                <div class="mb-4">
                    ...
                    <label for="icon"
                        ...
                        <img id="coverPreview"
                            src="{{ $theme->icon ? Helper::showImage($theme->icon, true) : '' }}"
                            class="position-absolute w-100 h-100 {{ $theme->icon ? '' : 'd-none' }}"
                            style="object-fit:cover;">

                        <span id="coverPlus" class="fs-1 {{ $theme->icon ? 'd-none' : '' }}">+</span>
                    </label>
                ...
                <div class="mb-3">
                    ...
                    <select name="status" class="form-control">
                        <option value="1" {{ $theme->status == '1' ? 'selected' : '' }}>Enable</option>
                        <option value="0" {{ $theme->status == '0' ? 'selected' : '' }}>Disable</option>
                    </select>
                </div>
