...
<div class="container">
    <div class="card">
        ...
        <div class="card-body">
            ...
            <form action="{{ route('entry.tag.edit', $entryTag->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="mb-3">
                    ...
                    <select name="visibility_type" id="visibilityType" class="form-control">
                        <option value="backend" {{ $entryTag->visibility_type === 'backend' ? 'selected' : '' }}>
                            Backend
                        </option>
                        <option value="in_app" {{ $entryTag->visibility_type === 'in_app' ? 'selected' : '' }}>
                            In App
                        </option>
                    </select>
                ...
                <div id="inAppFields" class="{{ $entryTag->visibility_type === 'in_app' ? '' : 'd-none' }}">

                    @php
                    $needcoins = old('needcoin', $entryTag->needcoin ?? []);
                    $validities = old('validity', $entryTag->validity ?? []);
                    ...
                    <!-- <div id="inAppFields"
                        class="{{ old('visibility_type', $entryTag->visibility_type) === 'in_app' ? '' : 'd-none' }}"> -->
                ...
                <div class="mb-4">
                    ...
                    <label for="icon"
                        ...
                        <img id="coverPreview"
                            src="{{ $entryTag->icon ? Helper::showImage($entryTag->icon, true) : '' }}"
                            class="position-absolute w-100 h-100 {{ $entryTag->icon ? '' : 'd-none' }}"
                            style="object-fit:cover;">

                        <span id="coverPlus" class="fs-1 {{ $entryTag->icon ? 'd-none' : '' }}">+</span>
                    </label>
                ...
                <div class="mb-4">
                    ...
                    @if(!empty($entryTag->gif))
                    <small class="d-block mt-1 text-muted">
                        Current File:
                        <a href="{{ Helper::showImage($entryTag->gif, true) }}" target="_blank">
                ...
                <div class="mb-3">
                    <label class="form-label fw-bold">Image Key <span class="text-danger">*</span></label>
                    <input type="text" name="img_key"
                        class="form-control @error('img_key') is-invalid @enderror"
                        value="{{ old('img_key', $entryTag->img_key) }}">
                </div>

                <div class="mb-3">
                    <label class="form-label fw-bold">Text Key <span class="text-danger">*</span></label>
                    <input type="text" name="text_key"
                        class="form-control @error('text_key') is-invalid @enderror"
                        value="{{ old('text_key', $entryTag->text_key) }}">
                </div>

                <div class="mb-3">
                    <label class="form-label fw-bold">Frame Key <span class="text-danger">*</span></label>
                    <input type="text" name="frame_key"
                        class="form-control @error('frame_key') is-invalid @enderror"
                        value="{{ old('frame_key', $entryTag->frame_key) }}">
                </div>

                <div class="mb-3">
                    <label class="form-label fw-bold">Status</label>
                    <select name="status" class="form-control">
                        <option value="1" {{ $entryTag->status == '1' ? 'selected' : '' }}>Enable</option>
                        <option value="0" {{ $entryTag->status == '0' ? 'selected' : '' }}>Disable</option>
                    </select>
                </div>

                <div class="d-flex justify-content-end gap-2">
                    <a href="{{ route('entry.tag') }}" class="btn btn-secondary">Cancel</a>
